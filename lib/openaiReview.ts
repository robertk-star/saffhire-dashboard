...
export async function runOpenAiReview(input: { caseRecord: any; chunks: any[] }): Promise<AiReviewOutput> {
  ...
  const instruction = isClientDisplayReview
    ? isCountySearch
      ...
      : "You are a SaffHire national record review assistant. For each pasted record, extract source, name, court/provider, county/state, case, charge, dates, and exact status/disposition wording. Decide whether a county search is required, why, and where to run it. Do not decide final reportability; the app applies SaffHire's default 7-year policy after extraction. Return valid JSON only."
