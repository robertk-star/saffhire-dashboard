...
export async function runOpenAiReview(input: { caseRecord: any; chunks: any[] }): Promise<AiReviewOutput> {
  ...
  const countyInstruction = "You are a SaffHire County Crim result review assistant. Review county-level records for identity, court level, charge, disposition, duplicate cases, supervision activity, age/scope, and final client decision. Identity is still most important. Output show, hide, or needs manual review with reasons. Return valid JSON only.";
