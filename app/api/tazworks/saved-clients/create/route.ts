...
export async function POST(request: Request) {
  ...
  if (!name || !clientGuid) return NextResponse.redirect(new URL("/tazworks/saved-clients?error=missing", request.url), 303);
  try {
    ...
    return NextResponse.redirect(new URL("/tazworks/saved-clients?created=1", request.url), 303);
  } catch {
    return NextResponse.redirect(new URL("/tazworks/saved-clients?error=create", request.url), 303);
  }
}
