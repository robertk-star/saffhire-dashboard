...
function ManualNationalDatabaseForm({ clientGuid, clientName, clientCode, orderGuid, searchGuid, fileNumber, personName, dob, defaultVendorText }: { clientGuid: string; clientName: string; clientCode: string; orderGuid: string; searchGuid: string; fileNumber: string; personName: string; dob: string; defaultVendorText: string }) {
  return <section className="card" style={{ padding: 22, marginBottom: 18 }}><h2 style={{ marginTop: 0 }}>National Database Analysis</h2><p style={{ color: "#5d687b" }}>Paste the vendor data here until TazWorks support confirms where the full National Alias address and alias output is exposed through the API.</p><form action="/api/analyze/quick" method="post" style={{ display: "grid", gap: 14 }}><input type="hidden" name="review_type" value="national_crim" /><input type="hidden" name="client_name" value={clientName} /><input type="hidden" name="client_code" value={clientCode} /><input type="hidden" name="order_guid" value={orderGuid} /><input type="hidden" name="search_guid" value={searchGuid} /><input type="hidden" name="reference_number" value={searchGuid} /><div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 12 }}><label><strong>File Number</strong><input name="file_number" defaultValue={fileNumber} style={{ width: "100%" }} /></label><label><strong>Name</strong><input name="person_name" defaultValue={personName} required style={{ width: "100%" }} /></label><label><strong>DOB</strong><input name="dob" defaultValue={dob} style={{ width: "100%" }} /></label></div><label><strong>Alias Information</strong><textarea name="aliases_text" placeholder="Paste aliases/name variations from the vendor here." style={{ minHeight: 110, width: "100%" }} /></label><label><strong>Address Information</strong><textarea name="address_text" placeholder="Paste address history/geography from the vendor here." style={{ minHeight: 140, width: "100%" }} /></label><label><strong>Vendor Records</strong><textarea name="pasted_text" defaultValue={defaultVendorText} required placeholder="Paste the records from the vendor here. This is the main record data AI will compare against the name, DOB, aliases, and address history above." style={{ minHeight: 320, width: "100%" }} /></label><button className="btn-primary" type="submit">Analyze</button></form></section>;
}
...
export default async function TazworksSearchResultPage({ params, searchParams }: { params: Promise<{ orderGuid: string; searchGuid: string }>; searchParams: Promise<{ clientGuid?: string; clientName?: string; clientCode?: string; fileNumber?: string; error?: string; detail?: string }> }) {
  ...
  const pageTitle = nationalAlias ? "National Database Analysis" : "TazWorks Search Result";
  return <><AppHeader user={user} /><main className="container-shell"><Link href={`/tazworks/orders/${orderGuid}?clientGuid=${encodeURIComponent(clientGuid)}&clientName=${encodeURIComponent(clientName)}&clientCode=${encodeURIComponent(clientCode)}&fileNumber=${encodeURIComponent(fileNumber)}`} style={{ color: "#0f3b5f", fontWeight: 800 }}>← Back to Searches</Link><h1>{pageTitle}</h1><div className="card" style={{ background: status.sandboxMode ? "#f8fafc" : "#fff7ed", borderColor: status.sandboxMode ? "#cbd5e1" : "#fdba74", padding: 16, marginBottom: 18 }}><strong>{status.sandboxMode ? "Sandbox Mode" : "Live TazWorks Mode"}</strong><p style={{ marginBottom: 0 }}>{nationalAlias ? "This page lets you paste National Database Alias vendor details and run Quick Analyze without changing the TazWorks order." : "Viewing this page only pulls data from TazWorks. Sending the result to an analyzer saves a copy to SaffHire and runs Quick Analyze. It does not change the TazWorks order."}</p></div><p style={{ color: "#5d687b" }}>Client: {clientLabel(clientName, clientCode)} | Order GUID: {orderGuid} | Search GUID: {searchGuid}{effectiveFileNumber ? ` | File #: ${effectiveFileNumber}` : ""}</p>{errorMessage ? <div className="card" style={{ borderColor: "#fda29b", background: "#fff5f5", color: "#b42318", fontWeight: 700, marginBottom: 18, padding: 16 }}>{errorMessage}</div> : null}{payload && nationalAlias ? <ManualNationalDatabaseForm clientGuid={clientGuid} clientName={clientName} clientCode={clientCode} orderGuid={orderGuid} searchGuid={searchGuid} fileNumber={effectiveFileNumber} personName={effectiveName} dob={effectiveDob} defaultVendorText={defaultVendorText} /> : null}{payload && !nationalAlias ? <><section className="card" style={{ padding: 18, marginBottom: 18 }}><p><strong>Search Type:</strong> {payload.type || "Unknown"}</p><p><strong>Status:</strong> {payload.status || "Unknown"}</p><p><strong>Result:</strong> {payload.result || "Unknown"}</p><p><strong>Suggested Analyzer:</strong> {analyzerName(suggestedReviewType)}</p><div style={{ borderTop: "1px solid #d7dee8", display: "grid", gap: 12, marginTop: 16, paddingTop: 16 }}><h2 style={{ margin: 0 }}>Send Result to Analysis</h2><p style={{ color: "#5d687b", margin: 0 }}>Choose the analyzer. National Criminal and County Criminal reviews use different decision trees.</p><div style={{ display: "flex", flexWrap: "wrap", gap: 12 }}><AnalysisForm reviewType="national_crim" clientGuid={clientGuid} clientName={clientName} clientCode={clientCode} orderGuid={orderGuid} searchGuid={searchGuid} fileNumber={effectiveFileNumber} /><AnalysisForm reviewType="county_search" clientGuid={clientGuid} clientName={clientName} clientCode={clientCode} orderGuid={orderGuid} searchGuid={searchGuid} fileNumber={effectiveFileNumber} /></div></div></section><section className="card" style={{ padding: 18 }}><h2 style={{ marginTop: 0 }}>Raw Result</h2><pre style={{ background: "#f8fafc", borderRadius: 12, maxHeight: 620, overflow: "auto", padding: 16, whiteSpace: "pre-wrap" }}>{JSON.stringify(payload, null, 2)}</pre></section></> : null}</main></>;
}
