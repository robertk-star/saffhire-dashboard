...
export async function runOpenAiReview(input: { caseRecord: any; chunks: any[] }): Promise<AiReviewOutput> {
  ...
  const instruction = isClientDisplayReview
    ? "You are a SaffHire record review assistant. Output must be operational and record-by-record. For each pasted record, identify the source, name, court, county/state, case, charge, offense date, file date when present, disposition date, status/disposition wording, whether a county search is needed, why, where to run, and the decision. Do not decide the final reportability rule yourself; the app applies SaffHire's default 7-year policy after your extraction. Return valid JSON only. Arrays must contain plain strings."
