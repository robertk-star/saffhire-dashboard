...
export async function POST(request: Request) {
  ...
  try {
    ...
    const reason = encodeURIComponent(String(err?.message || "import_failed").slice(0, 120));
    return NextResponse.redirect(new URL(`/tazworks?error=import_failed&detail=${reason}`, request.url), 303);
  }
}
