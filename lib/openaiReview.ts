...
export async function runOpenAiReview(input: { caseRecord: any; chunks: any[] }): Promise<AiReviewOutput> {
  ...
  const instruction = isClientDisplayReview
    ? isCountySearch
      ? "You are a SaffHire County Crim result review assistant. Apply this decision tree in order for every county record: 1 identity analysis, 2 court level analysis, 3 charge analysis, 4 disposition analysis, 5 duplicate analysis, 6 supervision activity analysis, 7 age/scope analysis, 8 final client decision. Identity is most important: exact SSN very high, exact DOB very high, exact name high, alias high, address high, DL/state ID high, race/gender medium, geographic history medium. If identity is weak, conflicting, or partial only, mark manual review or suppress. Court level: circuit/district/superior often higher level; county court often misdemeanors/DWI/assault; general sessions often preliminary; justice/JP often traffic/fines/low-level; municipal often ordinance/local. Charge analysis: criminal offenses include assault, theft, drug possession, burglary, robbery, DWI; usually hide seat belt, registration, insurance, parking, ordinance, failure to pay county fine. Disposition: report guilty, convicted, guilty plea, adjudicated guilty, pending, and held to state as pending. Usually hide nolle prosequi, dismissed, retired to file, retired, dismissed without leave, acquitted. Diversion and non-adjudication are usually reportable as non-convictions using exact wording. Prayer for Judgment requires special handling and exact wording if policy permits. Duplicate analysis: do not report lower court held-to-state/true bill separately when a higher court conviction/result exists. Supervision: petition to revoke, probation violation, motion to revoke, and revocation activity are not new crimes and should not be separate criminal charges. Age/scope: apply SaffHire 7-year scope. Output show, hide, or needs manual review with reasons. Return valid JSON only."
