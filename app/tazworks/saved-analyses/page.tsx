...
function matchesSearch(row: any, term: string) {
  if (!term) return true;
  const haystack = [fileNumber(row), row.person_name, row.dob, row.source_type, row.reference_number, orderGuid(row), searchGuid(row), row.full_text, row.pasted_text].map(text).join(" ").toLowerCase();
  return haystack.includes(term.toLowerCase());
}

export default async function SavedAnalysesPage({ searchParams }: { searchParams: Promise<{ type?: string; q?: string }> }) {
  const user = await requireUser(["admin", "supervisor", "analyzer"]);
  const params = await searchParams;
  const type = params.type || "all";
  const q = String(params.q || "").trim();
  const supabase = getSupabaseAdmin();
  let query = supabase.from("quick_reviews").select("id,review_type,person_name,dob,source_type,reference_number,created_at,created_by,full_text,pasted_text,result_json").in("review_type", ["national_crim", "county_search"]).order("created_at", { ascending: false }).limit(q ? 250 : 100);
  if (type === "national") query = query.eq("review_type", "national_crim");
  if (type === "county") query = query.eq("review_type", "county_search");
  const { data, error } = await query;
  const rows = (data || []).filter((row: any) => matchesSearch(row, q));
  return <><AppHeader user={user} /><main className="container-shell"><div style={{ alignItems: "center", display: "flex", justifyContent: "space-between", gap: 16, marginBottom: 18 }}><div><h1 style={{ margin: "0 0 8px" }}>Saved Analyses</h1><p style={{ color: "#5d687b", margin: 0 }}>Review prior National Crim and County Criminal analyses without rerunning AI.</p></div><Link className="btn-primary" href="/tazworks/current-orders">Current Orders</Link></div><section className="card" style={{ padding: 18, marginBottom: 18 }}><form method="get" action="/tazworks/saved-analyses" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 12, alignItems: "end" }}><label><span className="field-label">Search by file number or name</span><input className="field-input" name="q" defaultValue={q} placeholder="File # or applicant name" /></label><label><span className="field-label">Type</span><select className="field-input" name="type" defaultValue={type}><option value="all">All Analyses</option><option value="national">National Crim</option><option value="county">County Criminal</option></select></label><div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}><button className="btn-primary" type="submit">Search</button><Link className="btn-secondary" href="/tazworks/saved-analyses">Clear</Link></div></form></section>{error ? <section className="card" style={{ background: "#fff5f5", borderColor: "#fda29b", color: "#b42318", fontWeight: 700, padding: 16 }}>Saved analyses could not load.</section> : null}<section className="card table-wrap"><div style={{ padding: 18 }}><h2 style={{ margin: 0 }}>Saved Results</h2><p style={{ color: "#5d687b", marginBottom: 0 }}>{rows.length} saved analysis record{rows.length === 1 ? "" : "s"} shown{q ? ` for “${q}”` : ""}.</p></div><table><thead><tr><th>Date</th><th>Type</th><th>File #</th><th>Applicant</th><th>DOB</th><th>Order/Search</th><th>Ran By</th><th>Summary</th><th>Action</th></tr></thead><tbody>{rows.length ? rows.map((row: any) => <tr key={row.id}><td>{shortDate(row.created_at)}</td><td>{analysisType(row)}</td><td>{fileNumber(row)}</td><td>{row.person_name || ""}</td><td>{row.dob || ""}</td><td><div style={{ fontSize: 12, lineHeight: 1.5 }}>Order: {orderGuid(row) || "—"}<br />Search: {searchGuid(row) || "—"}</div></td><td>{row.created_by || ""}</td><td>{decisionSummary(row)}</td><td><Link style={{ color: "#0f3b5f", fontWeight: 800 }} href={`/analyze/${row.id}`}>Open</Link></td></tr>) : <tr><td colSpan={9}>{q ? "No saved analyses matched that search." : "No saved analyses found."}</td></tr>}</tbody></table></section></main></>;
}
