...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  await supabase.from("ai_reviews").insert({ case_id: caseRow.id, model_name: process.env.OPENAI_MODEL || "gpt-4.1-mini", review_summary: row.result_json?.review_summary || "Client display analysis", structured_output: row.result_json || {}, created_by_email: user.email });
