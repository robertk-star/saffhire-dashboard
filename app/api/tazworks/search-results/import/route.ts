...
function buildImportHeader(input: { label: string; clientGuid: string; clientCode: string; orderGuid: string; searchGuid: string; fileNumber: string; reviewType: ReviewType }) {
  return [
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const { data: existing } = await supabase.from("quick_reviews").select("id").eq("reference_number", payload?.orderSearchGuid || searchGuid).eq("review_type", reviewType).maybeSingle();
    ...
    const importContext = { clientGuid, clientName, clientCode, orderGuid, searchGuid, fileNumber, reviewType, analyzer: analyzerLabel(reviewType), importedAt: new Date().toISOString(), tazworksMutation: false };
    const storedPayload = { ...payload, _saffhireImportContext: importContext };
    const normalized = normalizeTazworksPayload(payload);
    const fullText = `${buildImportHeader({ label, clientGuid, clientCode, orderGuid, searchGuid, fileNumber, reviewType })}${buildCaseTextFromTazworksPayload(payload)}`;
    ...
    await supabase.from("tazworks_payloads").insert({ label: `API ${payload?.type || "Search Result"} - ${label}${fileNumber ? ` - File #${fileNumber}` : ""} - ${analyzerLabel(reviewType)}`, payload: storedPayload, applicant_name: normalized.applicantName || null, dob: normalized.dob || null, report_id: payload?.orderSearchGuid || searchGuid, record_count: normalized.recordCount || 0, imported_by_email: user.email });
